...
Реализована. Protobuf `v35.0` подключён, C++-код генерируется во время сборки,
...
- версия Protocol Buffers — `v35.0`, закреплённая неизменяемой меткой выпуска;
...
Первоначальная интеграция выполняется через CMake `FetchContent`. Запрещено
использовать `main` или другую незакреплённую ветку. Если на этапе `SERVER-001`
для объединения зависимостей Protobuf и gRPC потребуется пакетный менеджер,
изменение оформляется отдельным решением.
...
- [Protocol Buffers v35.0](https://github.com/protocolbuffers/protobuf/releases/tag/v35.0)
